...
function init() {
	...
	$this.routeManager.onRouteChange(function () {
		//$this.focusManager.getCurrent().focus(); // Shouldnt be neccessary, but avoids the "no focus before first refocus" bug
	});
	...
	// Dirty bugfix for the following undesired behaviour:
	// First option item of the the view which should be focused is not focused when the view is reloaded
//	setTimeout(function() {
//		$this.focusManager.getCurrent().focus();
//	})
}
