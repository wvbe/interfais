...
function InputManager(app) {
	...
	$this.init = function () {
		...
		// Focus next view
		if(configurableKeys.focusPrevious)
			$this.catch(configurableKeys.focusPrevious, function (key) {
				app.focusManager.shift(true);
			});

		// Focus previous view
		if(configurableKeys.focusNext)
			$this.catch(configurableKeys.focusNext, function (key) {
				app.focusManager.shift(false);
			});

		// Rerender entire layout
		if(configurableKeys.render)
			$this.catch(configurableKeys.render, function () {
				app.layout.prerender();
				app.layout.render();
			});

		// Exit application (hard)
		if(configurableKeys.exit)
			$this.catch(configurableKeys.exit, function (key) {
