...
function init() {
	...
	process.stdin.on('keypress', function (ch, key) {
		...
		if(!inputCatch)
			return;
		if(['backspace'].indexOf(key.sequence) >= 0)
			return inputCatch.emitter.emit(key.sequence, key);
...
function catchInput(regex) {
	if(inputCatch !== null)
		throw new Error('Previous input catch was not destroyed');
	...
	inputCatch.destroy = function() {
		inputCatch.emitter.removeAllListeners();
		inputCatch = null;
	};

	inputCatch.emitter.once('destroy', inputCatch.destroy);
