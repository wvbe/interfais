	...
	function viewFactory(cb) {
		...
		return function View (cell, args) {
			...
			function menuSetListeners() {
				if (ui.isInteractive()) {
					inputManager.on('menuPrevious', menuPrevious);
					inputManager.on('menuNext', menuNext);
					inputManager.on('menuConfirm', menuConfirm);
				}
			}

			function menuRemoveListeners() {
				if (ui.isInteractive()) {
					inputManager.off('menuPrevious', menuPrevious);
					inputManager.off('menuNext', menuNext);
					inputManager.off('menuConfirm', menuConfirm);
				}
			}
