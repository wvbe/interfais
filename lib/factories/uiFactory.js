...
uiFactory.prototype.option = function(name, callback) {
	...
	_queue.apply(self, [renderHelper.line, [' ' + command.getName() + ' '], function () {
		return {
...
uiFactory.prototype.input = function(name, callback, options) {
	...
	command.focus = function() {
		...
		try {
			...
			console.error(timesFocused + '/'+timesBlurred  +e.stack);
			process.exit();
		}

		focused = true;

		emitter.on('*', function(key) {
			value = value.slice(0, cursor) + key.sequence + value.slice(cursor, value.length);
			++cursor;
			self.render();
		});

		emitter.on('backspace', function() {
			if(cursor < 1)
				return;
		...
		emitter.on('left', function() {
			if(cursor > 0)
				--cursor;
			self.render();
		});

		emitter.on('right', function() {
			if(cursor < value.length)
				++cursor;
			self.render();
		});
