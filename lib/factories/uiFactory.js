...
uiFactory.prototype.option = function(name, callback) {
	...
	if (!self._menu)
		self._menu = new focusManager.FocusManager();
...
uiFactory.prototype.isInteractive = function () {
	return this._menu instanceof focusManager.FocusManager;
};
...
function _prerender() {
	...
	//	left: intInBetween(self._margin.x - scroll.x, 0)
	};
	var paddings = {
		top: intInBetween(self._padding.y - self._margin.y - scroll.y, 0, self._padding.y), // might be a mistake here?
	//	left: intInBetween(self._padding.x - (self._margins.x - scroll.x), 0, self._padding.x),
	};
	var clipping = {
		top: scroll.y,
		bottom: scroll.y + (self._box.height - margins.top - paddings.top) // viewport offset + viewport height
	};
