...
uiFactory.prototype.menu = function (children) {
	var self = this;
	...
	children.forEach(function (cmd, i) {

		if (!self._menu.getChildren().length)
			self._focused = cmd;

		self._menu.addChild(cmd);

		_queue.apply(self, [renderHelper.line, [' ' + cmd.toString() + ' '], function () {
			return {
				invert: self._focused === cmd ? true : false
			}
		}]);
	});
